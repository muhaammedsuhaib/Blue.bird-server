...
const user_validation = Joi.object({
  ...
  bio: Joi.string().optional().max(160).messages({
  ...
  followers: Joi.array().items(Joi.string()).optional().messages({
    "array.base": "Followers must be an array",
  }),

  following: Joi.array().items(Joi.string()).optional().messages({
    "array.base": "Following must be an array",
  }),

  posts: Joi.array().items(Joi.string()).optional().messages({
    "array.base": "Posts must be an array",
  }),

  likedPosts: Joi.array().items(Joi.string()).optional().messages({
    "array.base": "Liked posts must be an array",
  }),

  comments: Joi.array().items(Joi.string()).optional().messages({
    "array.base": "Comments must be an array",
  }),

  lastLogin: Joi.date().optional().messages({
