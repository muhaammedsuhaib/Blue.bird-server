...
const user_validation = joi_1.default.object({
    username: joi_1.default.string().required().trim().min(3).max(30).messages({
    ...
    bio: joi_1.default.string().optional().max(160).messages({
    ...
    followers: joi_1.default.array().items(joi_1.default.string()).optional().messages({
        "array.base": "Followers must be an array",
    }),
    following: joi_1.default.array().items(joi_1.default.string()).optional().messages({
        "array.base": "Following must be an array",
    }),
    posts: joi_1.default.array().items(joi_1.default.string()).optional().messages({
        "array.base": "Posts must be an array",
    }),
    likedPosts: joi_1.default.array().items(joi_1.default.string()).optional().messages({
        "array.base": "Liked posts must be an array",
    }),
    comments: joi_1.default.array().items(joi_1.default.string()).optional().messages({
        "array.base": "Comments must be an array",
    }),
    lastLogin: joi_1.default.date().optional().messages({
